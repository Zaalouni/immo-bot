// Généré automatiquement le 2026-04-04 12:16
const REVIEWS = {
  "date_calcul": "2026-04-04",
  "nb_modeles": 24,
  "note_moyenne_globale": 7.8,
  "marques_index": {
    "Audi": [
      "A3"
    ],
    "BMW": [
      "Serie 3"
    ],
    "Dacia": [
      "Duster"
    ],
    "Ford": [
      "Focus",
      "Puma"
    ],
    "Honda": [
      "CR-V"
    ],
    "Hyundai": [
      "Tucson"
    ],
    "Kia": [
      "Sportage"
    ],
    "Mazda": [
      "CX-5"
    ],
    "Mercedes": [
      "Classe C"
    ],
    "Nissan": [
      "Qashqai"
    ],
    "Opel": [
      "Astra"
    ],
    "Peugeot": [
      "308"
    ],
    "Renault": [
      "Clio"
    ],
    "Seat": [
      "Leon"
    ],
    "Skoda": [
      "Kodiaq",
      "Octavia"
    ],
    "Toyota": [
      "Corolla",
      "RAV4",
      "Yaris"
    ],
    "Volkswagen": [
      "Golf",
      "Polo",
      "Tiguan"
    ],
    "Volvo": [
      "XC40"
    ]
  },
  "reviews": [
    {
      "id": 6,
      "marque": "Audi",
      "modele": "A3",
      "annee_debut": 2020,
      "annee_fin": 2024,
      "note_globale": 7.5,
      "note_fiabilite": 7.0,
      "note_confort": 8.3,
      "note_consommation": 7.0,
      "cout_entretien": "Élevé",
      "problemes_connus": [
        "Problèmes logiciels MIB3 identiques à VW Golf",
        "Consommation réelle souvent supérieure à homologation WLTP"
      ],
      "points_forts": [
        "Finition intérieure premium",
        "Très bonne tenue de route",
        "Image de marque forte à la revente"
      ],
      "risques_achat": [
        "Budget entretien à prévoir (vidanges et pièces Audi premium)",
        "Vérifier historique entretien en concession agréée",
        "Attention aux frais de garantie hors réseau Audi"
      ],
      "sources": "TÜV Report 2023, ADAC 2022, L'Argus",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Coût d'entretien significativement plus élevé que VW/Skoda",
        "Même habitacle que la Golf pour un prix supérieur",
        "Coffre limité (325 L)"
      ],
      "fiabilite_resumee": "Premium bien fini mais coût d'entretien élevé. Même plateforme que la Golf avec une image de marque supplémentaire.",
      "consommation_mixte_l100": 6.5,
      "consommation_autoroute_l100": 7.8
    },
    {
      "id": 7,
      "marque": "BMW",
      "modele": "Serie 3",
      "annee_debut": 2019,
      "annee_fin": 2024,
      "note_globale": 7.6,
      "note_fiabilite": 6.8,
      "note_confort": 8.5,
      "note_consommation": 6.8,
      "cout_entretien": "Élevé",
      "problemes_connus": [
        "Problèmes de refroidissement sur certains moteurs B48",
        "Consommation d'huile sur B58 (surveillance requise)",
        "Système iDrive parfois lent"
      ],
      "points_forts": [
        "Plaisir de conduite exceptionnel",
        "Tenue de route de référence dans la catégorie",
        "Motorisations puissantes et raffinées",
        "Habitacle luxueux"
      ],
      "risques_achat": [
        "Exiger un historique entretien BMW exclusivement",
        "Vérifier l'état du système de refroidissement",
        "Budget réparation/entretien : compter 800–1 500 €/an",
        "Vérifier consommation d'huile avant achat"
      ],
      "sources": "TÜV Report 2023, ADAC 2023, Bimmerforums",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Fiabilité inférieure aux marques japonaises",
        "Consommation réelle élevée",
        "Pièces détachées coûteuses",
        "Suspension ferme en mode Sport"
      ],
      "fiabilite_resumee": "Plaisir de conduite excellent mais fiabilité en-deçà des Toyota/Mazda. Coût total de possession élevé.",
      "consommation_mixte_l100": 7.5,
      "consommation_autoroute_l100": 8.5
    },
    {
      "id": 12,
      "marque": "Dacia",
      "modele": "Duster",
      "annee_debut": 2018,
      "annee_fin": 2023,
      "note_globale": 7.4,
      "note_fiabilite": 7.6,
      "note_confort": 6.8,
      "note_consommation": 7.5,
      "cout_entretien": "Très faible",
      "problemes_connus": [
        "Boîte EDC (automatique) à surveiller : préférer boîte manuelle",
        "Quelques problèmes de corrosion sur plancher si véhicule sous-entretenu"
      ],
      "points_forts": [
        "Coût d'achat et d'entretien très bas",
        "4x4 disponible à prix accessible",
        "Mécanique simple = réparations bon marché",
        "Robustesse prouvée"
      ],
      "risques_achat": [
        "Fortement recommandé : boîte manuelle (EDC moins fiable)",
        "Vérifier absence de rouille sous caisse",
        "Vérifier entretien de la courroie de distribution"
      ],
      "sources": "TÜV Report 2023, ADAC 2022, Quechoisir",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Finition intérieure basique",
        "Confort limité sur mauvaise route",
        "Équipements électroniques minimalistes"
      ],
      "fiabilite_resumee": "Meilleur rapport coût total de possession du marché. Finition basique mais mécanique robuste et pièces bon marché.",
      "consommation_mixte_l100": 6.5,
      "consommation_autoroute_l100": 7.5
    },
    {
      "id": 20,
      "marque": "Ford",
      "modele": "Focus",
      "annee_debut": 2018,
      "annee_fin": 2024,
      "note_globale": 7.5,
      "note_fiabilite": 7.2,
      "note_confort": 7.6,
      "note_consommation": 7.5,
      "cout_entretien": "Moyen",
      "problemes_connus": [
        "Boîte PowerShift : transmissions défaillantes fréquentes — EVITER",
        "EcoBoost 1.0 : surveiller joint de culasse et pompe à eau"
      ],
      "points_forts": [
        "Excellent comportement routier (référence du segment)",
        "Moteur EcoBoost 1.0 économique",
        "Large choix en occasion (break, berline)",
        "Confort de suspension bien équilibré"
      ],
      "risques_achat": [
        "ABSOLUMENT éviter les versions boîte PowerShift",
        "Tester minutieusement les changements de vitesse",
        "Vérifier absence de surchauffe (joint de culasse)"
      ],
      "sources": "TÜV Report 2022, ADAC, WhatCar",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Boîte automatique PowerShift à éviter (fiabilité problématique)",
        "Consommation réelle EcoBoost souvent supérieure à l'annoncé"
      ],
      "fiabilite_resumee": "Compacte polyvalente avec une bonne tenue de route. Moteur EcoBoost 1.0 fiable mais demande de l'entretien.",
      "consommation_mixte_l100": 6.3,
      "consommation_autoroute_l100": 7.2
    },
    {
      "id": 11,
      "marque": "Ford",
      "modele": "Puma",
      "annee_debut": 2019,
      "annee_fin": 2024,
      "note_globale": 7.8,
      "note_fiabilite": 7.4,
      "note_confort": 7.6,
      "note_consommation": 7.8,
      "cout_entretien": "Moyen",
      "problemes_connus": [
        "Quelques retours sur des vibrations à froid sur le 3 cyl.",
        "Filtres à particules GPF à surveiller usage court-trajet"
      ],
      "points_forts": [
        "Moteur EcoBoost 1.0 mild-hybrid économique et plaisant",
        "Comportement routier dynamique",
        "Espace de coffre astucieux (MegaBox sous le plancher)",
        "Bon rapport équipement/prix"
      ],
      "risques_achat": [
        "Vérifier l'état du filtre à particules si usage urbain exclusif",
        "Préférer les modèles avec historique mixte ville/route"
      ],
      "sources": "Auto Express, What Car 2023, ADAC",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Habitabilité arrière en retrait vs concurrents",
        "Moteur 3 cylindres perceptible à l'accélération"
      ],
      "fiabilite_resumee": "Crossover compact très bien équilibré. Moteur EcoBoost mild-hybrid économique. Bon comportement routier.",
      "consommation_mixte_l100": 5.8,
      "consommation_autoroute_l100": 6.8
    },
    {
      "id": 17,
      "marque": "Honda",
      "modele": "CR-V",
      "annee_debut": 2018,
      "annee_fin": 2023,
      "note_globale": 7.8,
      "note_fiabilite": 8.0,
      "note_confort": 7.8,
      "note_consommation": 7.2,
      "cout_entretien": "Moyen",
      "problemes_connus": [
        "CVT : légère ruade à la reprise sur certains exemplaires",
        "Quelques retours sur des problèmes de climatisation (joints)"
      ],
      "points_forts": [
        "Fiabilité Honda reconnue",
        "Espace intérieur très généreux",
        "Moteur VTEC fiable",
        "Version hybride efficace en ville"
      ],
      "risques_achat": [
        "Vérifier présence d'un garage Honda agréé proche",
        "Tester la climatisation",
        "Carnet entretien Honda complet souhaitable"
      ],
      "sources": "Consumer Reports, ADAC 2022",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Réseau Honda moins dense en Luxembourg",
        "Interface multimédia datée sur premières années",
        "Consommation sur autoroute correcte mais pas exemplaire"
      ],
      "fiabilite_resumee": "Fiabilité japonaise au rendez-vous. Espace intérieur généreux. Moins courant en occasion = attention au réseau.",
      "consommation_mixte_l100": 7.0,
      "consommation_autoroute_l100": 8.0
    },
    {
      "id": 8,
      "marque": "Hyundai",
      "modele": "Tucson",
      "annee_debut": 2020,
      "annee_fin": 2024,
      "note_globale": 7.8,
      "note_fiabilite": 7.5,
      "note_confort": 8.2,
      "note_consommation": 7.8,
      "cout_entretien": "Moyen",
      "problemes_connus": [
        "Quelques retours sur une direction trop assistée",
        "Lags occasionnels de l'interface multimédia"
      ],
      "points_forts": [
        "Excellent niveau d'équipement de série",
        "Grand espace intérieur pour un SUV compact",
        "Design extérieur remarqué",
        "Garantie 5 ans constructeur"
      ],
      "risques_achat": [
        "Vérifier que la batterie hybride est encore sous garantie",
        "Tester le multimédia (mises à jour disponibles)"
      ],
      "sources": "TÜV Report 2023, Auto Plus, Caradisiac",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Consommation réelle au-dessus de l'annoncé sur autoroute",
        "Système multimédia parfois lent",
        "Rayon de braquage important"
      ],
      "fiabilite_resumee": "SUV familial moderne avec bon niveau d'équipement. Hybride bien intégré, fiabilité correcte.",
      "consommation_mixte_l100": 6.2,
      "consommation_autoroute_l100": 7.2
    },
    {
      "id": 9,
      "marque": "Kia",
      "modele": "Sportage",
      "annee_debut": 2022,
      "annee_fin": 2024,
      "note_globale": 7.9,
      "note_fiabilite": 7.6,
      "note_confort": 8.0,
      "note_consommation": 7.8,
      "cout_entretien": "Moyen",
      "problemes_connus": [
        "Quelques retours sur des creaks intérieur sur pièces plastique"
      ],
      "points_forts": [
        "Design intérieur double écran très réussi",
        "Grand coffre (540 L)",
        "Garantie 7 ans constructeur Kia",
        "Bonne répartition du poids"
      ],
      "risques_achat": [
        "Garantie 7 ans = atout majeur, vérifier que le 1er propriétaire l'a activée",
        "Tester la boîte DCT en usage urbain"
      ],
      "sources": "Auto Bild, Caradisiac 2023",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Peu de recul fiabilité sur cette génération (2022+)",
        "Boîte DCT 6 à surveiller sur gros kilométrages"
      ],
      "fiabilite_resumee": "Nouvelle génération très réussie. Design moderne, bon espace, moteur hybride efficace. Peu de recul encore.",
      "consommation_mixte_l100": 6.0,
      "consommation_autoroute_l100": 7.0
    },
    {
      "id": 10,
      "marque": "Mazda",
      "modele": "CX-5",
      "annee_debut": 2017,
      "annee_fin": 2024,
      "note_globale": 8.2,
      "note_fiabilite": 8.5,
      "note_confort": 8.3,
      "note_consommation": 7.2,
      "cout_entretien": "Faible à moyen",
      "problemes_connus": [
        "Très peu de problèmes recensés",
        "Quelques cas de rouille sous caisse sur modèles >2018 dans régions salées"
      ],
      "points_forts": [
        "Fiabilité excellente proche des Toyota",
        "Finition intérieure soignée",
        "Moteur SkyActiv économe et durable",
        "Comportement routier agréable"
      ],
      "risques_achat": [
        "Vérifier l'absence de rouille sous caisse (surtout si voiture venant d'un pays salant les routes)",
        "Carnet entretien Mazda recommandé"
      ],
      "sources": "TÜV Report 2023, ADAC 2022, Consumer Reports",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Pas de motorisation hybride/électrique disponible",
        "Légèrement énergivore sur autoroute vs concurrents hybrides",
        "Système connectivité un peu daté sur premières années"
      ],
      "fiabilite_resumee": "L'une des SUV les plus fiables du marché européen. Finition japonaise sérieuse, peu de problèmes.",
      "consommation_mixte_l100": 7.0,
      "consommation_autoroute_l100": 7.8
    },
    {
      "id": 18,
      "marque": "Mercedes",
      "modele": "Classe C",
      "annee_debut": 2021,
      "annee_fin": 2024,
      "note_globale": 7.4,
      "note_fiabilite": 6.5,
      "note_confort": 9.0,
      "note_consommation": 6.8,
      "cout_entretien": "Très élevé",
      "problemes_connus": [
        "Problèmes d'électronique complexe coûteux à réparer hors garantie",
        "Consommation d'huile sur moteurs M254",
        "Mise à jour logicielle MBUX parfois instable"
      ],
      "points_forts": [
        "Confort de référence dans la catégorie",
        "Habitacle ultra-luxueux",
        "Motorisations puissantes et raffinées",
        "Technologie embarquée de pointe (MBUX)"
      ],
      "risques_achat": [
        "Ne pas acheter hors garantie sans budget réparation conséquent",
        "Exiger historique entretien exclusivement en concession agréée Mercedes",
        "Vérifier l'état de la garantie constructeur ou extension"
      ],
      "sources": "TÜV Report 2023, ADAC 2023, Auto Motor Sport",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Fiabilité électronique inférieure aux marques japonaises",
        "Coût d'entretien très élevé (huiles spéciales, pièces d'origine)",
        "Dépréciation rapide sur les premières années"
      ],
      "fiabilite_resumee": "Confort et luxe inégalés mais fiabilité et coûts d'entretien problématiques sur le long terme.",
      "consommation_mixte_l100": 7.5,
      "consommation_autoroute_l100": 8.5
    },
    {
      "id": 16,
      "marque": "Nissan",
      "modele": "Qashqai",
      "annee_debut": 2021,
      "annee_fin": 2024,
      "note_globale": 7.7,
      "note_fiabilite": 7.5,
      "note_confort": 8.0,
      "note_consommation": 7.5,
      "cout_entretien": "Moyen",
      "problemes_connus": [
        "Très peu de problèmes recensés sur cette génération",
        "CVT : légère imprécision à la conduite sportive"
      ],
      "points_forts": [
        "Confort de suspension excellent",
        "Habitacle spacieux et bien fini",
        "Moteur e-Power unique : conduite silencieuse",
        "Fiabilité au-dessus de la moyenne"
      ],
      "risques_achat": [
        "Tester la CVT si usage autoroute fréquent",
        "Vérifier entretien Nissan officiel"
      ],
      "sources": "TÜV Report 2023, What Car, Auto Bild",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Boîte CVT sur certaines versions peu sportive",
        "Consommation e-Power décevante sur autoroute"
      ],
      "fiabilite_resumee": "SUV compact familial très équilibré. Moteur mild-hybrid propre, finition sérieuse, peu de problèmes.",
      "consommation_mixte_l100": 6.5,
      "consommation_autoroute_l100": 7.5
    },
    {
      "id": 19,
      "marque": "Opel",
      "modele": "Astra",
      "annee_debut": 2022,
      "annee_fin": 2024,
      "note_globale": 7.4,
      "note_fiabilite": 7.0,
      "note_confort": 7.8,
      "note_consommation": 7.3,
      "cout_entretien": "Moyen",
      "problemes_connus": [
        "Moteur PureTech 1.2 : surveiller chaîne de distribution (comme Peugeot 308)"
      ],
      "points_forts": [
        "Design extérieur moderne",
        "Équipements très complets de série",
        "Confort de suspension bien géré",
        "Version électrique disponible"
      ],
      "risques_achat": [
        "Vérifier état chaîne distribution PureTech",
        "Peu de recul disponible — préférer si sous garantie constructeur"
      ],
      "sources": "Auto Bild 2023, Caradisiac",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Partage le moteur PureTech de Peugeot — mêmes problèmes potentiels",
        "Peu de recul sur la fiabilité long terme (nouveau depuis 2022)"
      ],
      "fiabilite_resumee": "Nouvelle Astra très bien équipée. Partage les bases Peugeot 308 — même moteur PureTech, mêmes vigilances.",
      "consommation_mixte_l100": 6.2,
      "consommation_autoroute_l100": 7.0
    },
    {
      "id": 13,
      "marque": "Peugeot",
      "modele": "308",
      "annee_debut": 2021,
      "annee_fin": 2024,
      "note_globale": 7.5,
      "note_fiabilite": 7.0,
      "note_confort": 8.0,
      "note_consommation": 7.3,
      "cout_entretien": "Moyen",
      "problemes_connus": [
        "IMPORTANT : Moteur PureTech 1.2 (2014–2022) connu pour casse de la courroie de distribution",
        "PSA a étendu la garantie sur ce problème — vérifier si réparé",
        "Problèmes de joints de culasse sur PureTech premières années"
      ],
      "points_forts": [
        "Design intérieur très réussi (volant tronqué, digital cockpit)",
        "Confort de suspension bien dosé",
        "Moteur PureTech 1.2 économique"
      ],
      "risques_achat": [
        "CRITIQUE : Exiger preuve de remplacement de la chaîne de distribution PureTech",
        "Préférer les versions 2022+ ou les versions diesel/électrique",
        "Vérifier historique en réseau Peugeot agréé"
      ],
      "sources": "ADAC 2023, Quechoisir, 60 Millions de Consommateurs",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "PureTech 1.2 : chaîne de distribution fragile sur premières années",
        "Fiabilité électronique perfectible",
        "Interface i-Cockpit pas toujours intuitive"
      ],
      "fiabilite_resumee": "Design intérieur primé. Bonne voiture mais la fiabilité des PSA reste un point d'attention, surtout sur les versions hybrides rechargeables.",
      "consommation_mixte_l100": 6.2,
      "consommation_autoroute_l100": 7.0
    },
    {
      "id": 14,
      "marque": "Renault",
      "modele": "Clio",
      "annee_debut": 2019,
      "annee_fin": 2024,
      "note_globale": 7.5,
      "note_fiabilite": 7.2,
      "note_confort": 7.5,
      "note_consommation": 7.8,
      "cout_entretien": "Faible à moyen",
      "problemes_connus": [
        "Boîte EDC : à éviter si usage très urbain (embrayage usé rapidement)",
        "TCe 100 : surveiller consommation d'huile"
      ],
      "points_forts": [
        "Moteur TCe 1.0 globalement fiable",
        "Habitacle bien fini pour le segment",
        "Large réseau Renault pour entretien",
        "Confort correct"
      ],
      "risques_achat": [
        "Privilégier boîte manuelle à l'EDC",
        "Vérifier l'huile moteur avant achat",
        "Tester la boîte EDC en conditions stop-and-go"
      ],
      "sources": "ADAC 2022, TÜV Report 2023, Caradisiac",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Boîte EDC (double embrayage) moins fiable que la manuelle",
        "Multimédia Easy Link parfois lent",
        "Coffre limité (391 L)"
      ],
      "fiabilite_resumee": "Citadine bien équilibrée. Moteur TCe 1.0 plus fiable que le PureTech Peugeot. Bonne polyvalence.",
      "consommation_mixte_l100": 5.5,
      "consommation_autoroute_l100": 6.5
    },
    {
      "id": 15,
      "marque": "Seat",
      "modele": "Leon",
      "annee_debut": 2020,
      "annee_fin": 2024,
      "note_globale": 7.6,
      "note_fiabilite": 7.2,
      "note_confort": 7.8,
      "note_consommation": 7.3,
      "cout_entretien": "Moyen",
      "problemes_connus": [
        "Multimédia MIB3 identique à VW — mêmes bugs",
        "DSG7 à surveiller en usage urbain intensif"
      ],
      "points_forts": [
        "Meilleur rapport équipement/prix du groupe VW",
        "Disponible en break (ST) très pratique",
        "Comportement routier dynamique"
      ],
      "risques_achat": [
        "Préférer modèles 2022+ pour les bugs logiciels résolus",
        "Vérifier rappels VW/Seat effectués"
      ],
      "sources": "TÜV Report 2023, ADAC 2022",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Mêmes bugs logiciels que Golf Mk8",
        "Moins valorisé à la revente que VW"
      ],
      "fiabilite_resumee": "Même plateforme que la Golf Mk8 avec un meilleur rapport prix/équipement. Mêmes points de vigilance électroniques.",
      "consommation_mixte_l100": 6.0,
      "consommation_autoroute_l100": 7.0
    },
    {
      "id": 22,
      "marque": "Skoda",
      "modele": "Kodiaq",
      "annee_debut": 2017,
      "annee_fin": 2023,
      "note_globale": 7.8,
      "note_fiabilite": 7.5,
      "note_confort": 8.4,
      "note_consommation": 7.2,
      "cout_entretien": "Moyen",
      "problemes_connus": [
        "Boîte DSG 7 : à-coups et saccades en petite vitesse (surtout 2017–2019, TSB disponible)",
        "Module de direction assistée électrique — quelques pannes signalées",
        "Capteurs de stationnement défaillants (problème commun MQB)"
      ],
      "points_forts": [
        "7 places disponibles — rare dans ce segment de prix",
        "Habitacle spacieux et bien fini pour le prix",
        "Coffre modulable généreux (270–2 005 L)",
        "Bonne valeur d'achat occasion",
        "Version 4x4 4Drive disponible"
      ],
      "risques_achat": [
        "Exiger historique entretien DSG (vidange huile boîte tous les 60 000 km)",
        "Essai routier obligatoire avec la boîte DSG — tester les manœuvres lentes",
        "Vérifier les logiciels : mise à jour MIB recommandée",
        "Préférer les versions 2020+ pour la DSG améliorée"
      ],
      "sources": "TÜV Report 2023, Fiabilité-Auto.com, ADAC 2022, forums Skoda France",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Boîte DSG 7 rapports parfois capricieuse sur les 2017–2019",
        "Fiabilité inférieure aux Toyota/Mazda",
        "Consommation diesel réelle souvent >7.5 L/100",
        "Interface MIB2 vieillissante"
      ],
      "fiabilite_resumee": "Grand SUV familial 7 places solide sur la plateforme MQB Volkswagen. Fiabilité correcte mais surveillance de la DSG et des problèmes logiciels.",
      "consommation_mixte_l100": 7.5,
      "consommation_autoroute_l100": 6.2
    },
    {
      "id": 5,
      "marque": "Skoda",
      "modele": "Octavia",
      "annee_debut": 2020,
      "annee_fin": 2024,
      "note_globale": 8.0,
      "note_fiabilite": 7.8,
      "note_confort": 8.0,
      "note_consommation": 7.5,
      "cout_entretien": "Moyen",
      "problemes_connus": [
        "DSG7 à surveiller si beaucoup de conduite urbaine",
        "Quelques cas de gel du multimédia"
      ],
      "points_forts": [
        "Grand coffre (600 L) excellent pour les familles",
        "Meilleur rapport espace/prix du segment",
        "Finition sérieuse",
        "Moins de bugs que la Golf Mk8"
      ],
      "risques_achat": [
        "Vérifier état boîte DSG à hauts kilométrages",
        "Préférer version avec entretien officiel Skoda"
      ],
      "sources": "TÜV Report 2023, ADAC 2023",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Dessin intérieur un peu terne",
        "Moteur 1.5 TSI peut consommer plus que prévu"
      ],
      "fiabilite_resumee": "Très bon rapport qualité/prix. Partage la plateforme Golf avec moins de bugs et plus d'espace.",
      "consommation_mixte_l100": 6.0,
      "consommation_autoroute_l100": 7.0
    },
    {
      "id": 1,
      "marque": "Toyota",
      "modele": "Corolla",
      "annee_debut": 2019,
      "annee_fin": 2024,
      "note_globale": 8.5,
      "note_fiabilite": 9.2,
      "note_confort": 7.8,
      "note_consommation": 8.8,
      "cout_entretien": "Faible",
      "problemes_connus": [
        "Consommation d'huile légèrement élevée sur certains moteurs 1.8 (rares)",
        "Interface multimédia lente sur modèles 2019–2021"
      ],
      "points_forts": [
        "Fiabilité parmi les meilleures de sa catégorie",
        "Consommation maîtrisée en ville (3,5–4,5 L/100)",
        "Faibles coûts d'entretien",
        "Bonne valeur de revente"
      ],
      "risques_achat": [
        "Exiger un historique entretien Toyota certifié",
        "Vérifier l'état de la batterie hybride si >150 000 km"
      ],
      "sources": "TÜV Report 2023, ADAC 2023, JD Power Europe 2022",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Boîte CVT bruyante à haut régime sur autoroute",
        "Insonorisation perfectible",
        "Coffre un peu juste pour une berline"
      ],
      "fiabilite_resumee": "L'une des voitures les plus fiables du marché. Moteur hybride éprouvé et sans surprise.",
      "consommation_mixte_l100": 4.5,
      "consommation_autoroute_l100": 5.2
    },
    {
      "id": 21,
      "marque": "Toyota",
      "modele": "RAV4",
      "annee_debut": 2019,
      "annee_fin": null,
      "note_globale": 8.6,
      "note_fiabilite": 9.0,
      "note_confort": 8.2,
      "note_consommation": 8.5,
      "cout_entretien": "Faible",
      "problemes_connus": [
        "Quelques cas de fissures sur le pare-brise (rares, sous garantie)",
        "Consommation d'huile légèrement élevée sur certains XA50 (surveiller)"
      ],
      "points_forts": [
        "Fiabilité Toyota légendaire — parmi les meilleures du segment SUV",
        "Consommation hybride très basse (4.5–6 L/100 en usage mixte)",
        "Faibles coûts d'entretien vs concurrents premium",
        "Excellente valeur de revente",
        "Très bon comportement en conduite mixte ville/autoroute"
      ],
      "risques_achat": [
        "Vérifier historique entretien Toyota exclusivement",
        "Contrôler l'état de la batterie hybride haute tension si >120 000 km",
        "Inspecter la carrosserie : véhicule très demandé, prix parfois gonflés"
      ],
      "sources": "TÜV Report 2023, ADAC 2023, What Car? 2023, Fiabilité Toyota France",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Insonorisation inférieure aux BMW/Volvo de même gamme",
        "Boîte CVT peu sportive",
        "Coffre (580 L) dans la moyenne, non exceptionnel",
        "Interface multimédia vieillissante sur les 2019–2021"
      ],
      "fiabilite_resumee": "Référence absolue en fiabilité SUV. Moteur hybride 2.5L sans problème majeur signalé. Valeur de revente exceptionnelle au Luxembourg.",
      "consommation_mixte_l100": 5.5,
      "consommation_autoroute_l100": 6.8
    },
    {
      "id": 2,
      "marque": "Toyota",
      "modele": "Yaris",
      "annee_debut": 2020,
      "annee_fin": 2024,
      "note_globale": 8.3,
      "note_fiabilite": 9.0,
      "note_confort": 7.5,
      "note_consommation": 9.0,
      "cout_entretien": "Faible",
      "problemes_connus": [
        "Quelques retours sur la consommation réelle plus haute qu'annoncée sur autoroute"
      ],
      "points_forts": [
        "Consommation exceptionnelle en ville (3–4 L/100)",
        "Fiabilité Toyota reconnue",
        "Compacte et manœuvrable",
        "Équipements modernes"
      ],
      "risques_achat": [
        "Vérifier l'état de la batterie si usage intensif ville uniquement",
        "Historique entretien obligatoire"
      ],
      "sources": "TÜV Report 2023, ADAC 2022, Caradisiac",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Habitabilité arrière limitée",
        "Pas idéale pour longs trajets autoroute",
        "Coffre petit (286 L)"
      ],
      "fiabilite_resumee": "Citadine hybride ultra-fiable. Idéale pour la ville, redoutablement efficace en consommation.",
      "consommation_mixte_l100": 4.2,
      "consommation_autoroute_l100": 5.5
    },
    {
      "id": 3,
      "marque": "Volkswagen",
      "modele": "Golf",
      "annee_debut": 2020,
      "annee_fin": 2024,
      "note_globale": 7.6,
      "note_fiabilite": 7.0,
      "note_confort": 8.2,
      "note_consommation": 7.2,
      "cout_entretien": "Moyen",
      "problemes_connus": [
        "Plantages du système multimédia MIB3 (mises à jour disponibles)",
        "Problèmes de climatisation sur certains lots",
        "DSG7 à surveiller à hauts kilométrages"
      ],
      "points_forts": [
        "Habitacle de qualité premium",
        "Excellente tenue de route",
        "Large choix de motorisations",
        "Bonne valeur sur le marché de l'occasion"
      ],
      "risques_achat": [
        "Préférer les modèles 2022+ (bugs logiciels corrigés)",
        "Vérifier que les rappels VW ont été effectués",
        "Tester le multimédia et la climatisation avant achat"
      ],
      "sources": "TÜV Report 2023, ADAC 2023, Auto Motor Sport",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Bugs logiciels fréquents sur modèles 2020–2021",
        "Interface tactile peu intuitive",
        "Coût des réparations électroniques élevé"
      ],
      "fiabilite_resumee": "Bonne voiture mais la Mk8 a souffert de bugs électroniques à ses débuts. Les versions récentes (2022+) sont plus abouties.",
      "consommation_mixte_l100": 6.5,
      "consommation_autoroute_l100": 7.5
    },
    {
      "id": 4,
      "marque": "Volkswagen",
      "modele": "Polo",
      "annee_debut": 2017,
      "annee_fin": 2024,
      "note_globale": 7.5,
      "note_fiabilite": 7.3,
      "note_confort": 7.2,
      "note_consommation": 7.5,
      "cout_entretien": "Moyen",
      "problemes_connus": [
        "Chaîne de distribution TSI à surveiller sur premières années",
        "Quelques cas de consommation d'huile 1.0 TSI"
      ],
      "points_forts": [
        "Finition soignée pour le segment",
        "Bonne habitabilité pour une citadine",
        "Moteur TSI 1.0 fiable et économe"
      ],
      "risques_achat": [
        "Préférer moteur 1.0 TSI 110ch ou 1.5 TSI",
        "Vérifier l'entretien de la chaîne de distribution"
      ],
      "sources": "TÜV Report 2023, ADAC 2022",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Moteur 1.0 TSI 95ch un peu juste sur autoroute",
        "Insonorisation moyenne"
      ],
      "fiabilite_resumee": "Citadine solide et polyvalente. Moins de problèmes électroniques que la Golf Mk8.",
      "consommation_mixte_l100": 5.5,
      "consommation_autoroute_l100": 6.5
    },
    {
      "id": 24,
      "marque": "Volkswagen",
      "modele": "Tiguan",
      "annee_debut": 2016,
      "annee_fin": 2023,
      "note_globale": 7.6,
      "note_fiabilite": 7.1,
      "note_confort": 8.1,
      "note_consommation": 7.0,
      "cout_entretien": "Moyen",
      "problemes_connus": [
        "Boîte DSG 7 : saccades et claquements à basse vitesse (surtout 2016–2019)",
        "Injecteurs diesel sur TDI 2.0 — surveiller",
        "Capteurs de pluie et de luminosité défaillants (commun VW)"
      ],
      "points_forts": [
        "Intérieur spacieux et bien fini",
        "Bonne dynamique de conduite",
        "4Motion (4x4) disponible sur tous les moteurs",
        "Bonne valeur sur le marché de l'occasion",
        "Version Allspace 7 places disponible"
      ],
      "risques_achat": [
        "Vérifier la boîte DSG avec un test en circulation dense",
        "Historique entretien VW agréé obligatoire",
        "Contrôle OBD2 recommandé pour codes erreur latents",
        "Préférer les versions 2020+ (mises à jour DSG et logiciel)"
      ],
      "sources": "TÜV Report 2023, ADAC 2023, Fiabilité-Auto.com, Auto Bild",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Boîte DSG 7 : même problématique que Skoda (pré-2020)",
        "Fiabilité électronique inférieure aux Toyota/Mazda",
        "Consommation réelle souvent 10–15% au-dessus WLTP",
        "Plastiques durs en bas de gamme de finition"
      ],
      "fiabilite_resumee": "SUV polyvalent bien construit mais quelques points de vigilance sur la DSG et le logiciel. Bon rapport espace/prix en occasion.",
      "consommation_mixte_l100": 7.8,
      "consommation_autoroute_l100": 7.0
    },
    {
      "id": 23,
      "marque": "Volvo",
      "modele": "XC40",
      "annee_debut": 2018,
      "annee_fin": null,
      "note_globale": 7.9,
      "note_fiabilite": 7.3,
      "note_confort": 8.8,
      "note_consommation": 7.0,
      "cout_entretien": "Élevé",
      "problemes_connus": [
        "Système SENSUS (pre-2021) — plantages et lenteurs",
        "Correcteur électronique de stabilité (DSTC) parfois sensible — calibrage nécessaire",
        "Petits problèmes de joints d'étanchéité sur les B3/B4 (2020+)",
        "Tapis de coffre mal fixé — problème cosmétique répandu"
      ],
      "points_forts": [
        "Sécurité 5 étoiles Euro NCAP — parmi les meilleurs du marché",
        "Confort et qualité d'habitacle premium",
        "Design intérieur Scandinave remarquable",
        "Système d'aide à la conduite parmi les plus avancés",
        "Version 100% électrique disponible (Recharge)"
      ],
      "risques_achat": [
        "Chiffrage entretien Volvo préalable : compter 800–1 200 €/an",
        "Vérifier historique entretien réseau agréé uniquement",
        "Tester le système SENSUS : fluidité et connectivité smartphone",
        "Préférer les 2021+ avec le nouveau système Google Android Automotive"
      ],
      "sources": "Euro NCAP 2022, TÜV Report 2023, ADAC 2023, What Car? Awards",
      "derniere_maj": "2026-04-04",
      "points_faibles": [
        "Coût d'entretien Volvo élevé hors réseau agréé",
        "Moteurs T3/T4 essence parfois gourmands en réalité",
        "Fiabilité électronique inférieure aux japonaises",
        "Interface SENSUS critiquée pour sa lenteur (pre-2021)"
      ],
      "fiabilite_resumee": "SUV compact premium très confortable, sécurité maximale. Fiabilité correcte mais coûts entretien élevés et quelques problèmes électroniques sur les premières séries.",
      "consommation_mixte_l100": 7.8,
      "consommation_autoroute_l100": 8.5
    }
  ]
};
